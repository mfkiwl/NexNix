...
BOOT_BIOS_LDSCRIPT := $(BOOT_SRCDIR)/mach/bios/arch/$(GLOBAL_MACH)/link.ld
...
$(eval $(call LD_TEMPLATE,$(BOOT_OUTPUTNAME),$(BOOT_OBJFILES) $(BOOT_ASOBJS),\
		$(LIBK_OUTPUTNAME) $(BOOT_BIOS_LDSCRIPT),nexboot,$(BOOT_LINKLIBS),$(BOOT_BIOS_LDFLAGS)))
$(eval $(call CC_TEMPLATE,$(BOOT_OBJDIR),$(BOOT_SRCDIR),$(BOOT_BIOS_CFLAGS),$(GLOBAL_INCDIR)/boot/hdrstate))
...
ifeq ($(GLOBAL_MACH),i386)
	@$(MINGW_CC) -L $(GLOBAL_PREFIX)/lib $(BOOTEFI_LDFLAGS) $(BOOTEFI_OBJFILES) -lk -o $(BOOTEFI_OUTPUTNAME)
endif
	@echo "[INSTALL] Installing $(notdir $@)"
	@install $(BOOTEFI_OUTPUTNAME) $(GLOBAL_OUTPUTDIR)/nexboot.efi

# Compiler target
$(BOOTEFI_OBJDIR)/boot/mach/efi/%.c.o: $(BOOTEFI_SRCDIR)/%.c $(GLOBAL_INCDIR)/hdrstate \
									$(GLOBAL_INCDIR)/boot/hdrstate Makefile
	@echo "[CC] Building $<"
	@mkdir -p $(dir $@)
	@$(MINGW_CC) -MD $(GLOBAL_CFLAGS) $(BOOTEFI_CFLAGS) $(BOOTEFI_CFLAGS_$(GLOBAL_MACH)) -c $< -o $@
